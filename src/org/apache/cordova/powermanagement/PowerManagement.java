...
public class PowerManagement extends CordovaPlugin {
		...
		try {
			if( action.equals("acquire") ) {
				if( args.length() > 0 && args.getBoolean(0) ) {
					...
					result = this.acquire( PowerManager.PARTIAL_WAKE_LOCK, -1 );
				}
				else {
					result = this.acquire( PowerManager.FULL_WAKE_LOCK, -1 );
				}
			...
			} else if( action.equals("acquireWithExpireTime") ) {
				Log.d("PowerManagementPlugin", "Only dim lock with expire time" );
				if( args.length() > 0 ) {
					long time = args.getLong(0);
					Log.d("PowerManagementPlugin", "dim for - " + time );
					result = this.acquire( PowerManager.PARTIAL_WAKE_LOCK, time );
				}
				else this.acquire( PowerManager.PARTIAL_WAKE_LOCK, -1 );
			}
		}
	 ...
	 */
	private PluginResult acquire( int p_flags, long time ) {
		PluginResult result = null;
